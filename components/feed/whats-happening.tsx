        ...
        <div className="space-y-4">
          {mockTrends.map((trend) => (
            <div key={trend.id} className="space-y-1">
              ...
              <Link href={`/trends/${trend.id}`} className="block font-semibold hover:underline">
                {trend.title}
              </Link>
              <p className="text-xs text-muted-foreground">{trend.engagementCount} people engaged</p>
            </div>
          ))}
        </div>
      </div>
